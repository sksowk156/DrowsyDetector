...
class AnalyzeService : LifecycleService() {
    ...
    @Inject
    lateinit var staticsRepository: AnalyzerResultRepository

    @Inject
    lateinit var musicHelperService: MusicHelperService
    ...
    private fun startMusic(defaultMode: Boolean) {
        musicHelperService.releaseMediaPlayer()

        if (defaultMode) {
            musicHelperService.setResMusic()
        } else {
            if (musicList.size > 0) musicHelperService.setMyMusic(musicList)
        }
    }
...
//        })
    ...
    private fun getMainActivityPendingIntent(): PendingIntent = PendingIntent.getActivity(
        this, 199,
//        Intent(this, requireActivity::class.java).apply {
//            this.action = ACTION_SHOW_ANALYZING_FRAGMENT }
    ...
    fun calRatio(
        ...
        val rightUpper = landmark[159].position
        val rightLower = landmark[145].position

        val leftUpper = landmark[386].position
        val leftLower = landmark[374].position

        val widthLower = (calDist(rightLower, leftLower)) * leftRightSec
