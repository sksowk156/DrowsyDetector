...
import com.paradise.analyze.databinding.FragmentAnalyzesBinding
...
class AnalyzesFragment : BaseFragment<FragmentAnalyzesBinding>(FragmentAnalyzesBinding::inflate),
    ...
    private var analyzesService: AnalyzesService? = null
    var isServiceBounded = false

    private lateinit var overlay: PreviewOverlayView
    ...
    private fun subscribeUserMusicList() =
        viewLifecycleOwner.launchWithRepeatOnLifecycle(state = Lifecycle.State.STARTED) {
            analyzesViewModel.music.collect { musicList ->
    ...
    private fun startCamera(
        ...
        cameraHelper.startAnalyze(binding.analyzeViewFinder) { resultDetector, resultMesh ->
            if (this@AnalyzesFragment.isAdded) {
                ...
                if (analyzesService?.standard == null) {
                    checkHeadPoseInNoStandard(eyeRatio, leftRightAngle, upDownAngle)
                } else {
                    analyzesService?.startRecording()
                    val eyeState = eyeRatio / analyzesService?.standard!! // 눈 상태
                    checkHeadPoseInStandard(leftRightAngle, upDownAngle)
                    // 비율이 제한을 벗어났을 때
                    if (eyeState <= DROWSY_THREDHOLD) {
                        // 눈 깜빡임 카운팅
                        analyzesService?.checkEyeWink(eyeState, resultDetector)
                        ...
                        if (resultDetector.smilingProbability!! <= SMILE_THREDHOLD && !(musicHelper?.isPrepared?.value)!! && sttTtsController?.checkTtsSttHelperReady()!!) {
                            ...
                            if (timeCheckDrowsy != null) {
                                ...
                                if (maintainTime > TIME_THREDHOLD) {
                                    if (analyzesViewModel.checkDrowsy) { // 한번만 동작하게
                                        analyzesViewModel.checkDrowsy = false
                                        analyzesService?.countUpDrowsyCount()
                                        // 경고음
                                        startMusic()
    ...
    private fun setEyeStateInOpen() {
        analyzesService?.eyeClosed = false
        if (isInDrowsyState) {
            analyzesViewModel.checkDrowsy = true
            binding.analyzeTextDrowsycheck.visibility = View.INVISIBLE
    ...
    private fun getJobForType(detectType: String, count: Int, location: Location) {
        when (detectType) {
            "ParkingLot" -> {
                jobGetParkingLot = analyzesViewModel.getALLNearParkingLot(
            ...
            "Shelter" -> {
                jobGetShelter = analyzesViewModel.getNearShelter(
                    boundingBox = getBoundingBox(
                        location.latitude, location.longitude, DEFAULT_RADIUSKM + (5.0 * count)
                    )
                )
            ...
            "Rest" -> {
                jobGetRest = analyzesViewModel.getNearRest(
                    boundingBox = getBoundingBox(
                        location.latitude, location.longitude, DEFAULT_RADIUSKM + (5.0 * count)
                    )
                )
    ...
    private fun subscribeSortedAll() = viewLifecycleOwner.launchWithRepeatOnLifecycle(
        ...
        analyzesViewModel.sortedAll.collect { items ->
            var result = ""
    ...
    private fun subscribeParkingLotData() = viewLifecycleOwner.launchWithRepeatOnLifecycle(
        ...
        analyzesViewModel.parkingLots.collect {
            when (it) {
    ...
    private fun subscribeShleterData() = viewLifecycleOwner.launchWithRepeatOnLifecycle(
        ...
        analyzesViewModel.shelters.collect {
            when (it) {
                is UiState.Uninitialized -> {}
                is UiState.Loading -> {
    ...
    private fun subscribeRestData() = viewLifecycleOwner.launchWithRepeatOnLifecycle(
        ...
        analyzesViewModel.rests.collect {
            when (it) {
                is UiState.Uninitialized -> {}
                is UiState.Loading -> {
            ...
            when (T::class) {
                parkingLotItem::class -> {
                    jobSortedParkingLot = handleDataSorting(jobSortedParkingLot) {
                        analyzesViewModel.sortParkingLots(
                ...
                shelterItem::class -> {
                    jobSortedShelter = handleDataSorting(jobSortedShelter) {
                        analyzesViewModel.sortShelters(
                ...
                restItem::class -> {
                    jobSortedRest = handleDataSorting(jobSortedRest) {
                        analyzesViewModel.sortRests(nowLocation, data as List<restItem>)
                    }
                }
            ...
            with(binding) {
                ...
                if (standardRatioList.size > 0) {
                    ...
                    if (!avg.isNaN()) {
                        ...
                        analyzesService?.insertRecord(analyzesService?.currentAnayzeResult!!)
                    } else {
