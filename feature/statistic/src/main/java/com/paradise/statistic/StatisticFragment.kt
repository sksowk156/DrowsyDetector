...
import com.paradise.statistic.databinding.FragmentStatisticBinding
        ...
        with(binding) {
            ...
            for (i in winkList.indices) {
                barEntries.add(
                    BarEntry(
                        (i + 1).toFloat(),
                        winkList[i].value!!.toFloat()
                    )
                ) // 1(간격)부터 시작
            }
            val barDataSet = BarDataSet(barEntries, "눈 깜빡임 횟수")
            barDataSet.color =
                androidx.core.content.ContextCompat.getColor(requireContext(), R.color.primary1)
            ...
            lineDataSet.color = android.graphics.Color.YELLOW
            ...
            chartStatisticAnalyzeresult.setBackgroundColor(android.graphics.Color.WHITE)
