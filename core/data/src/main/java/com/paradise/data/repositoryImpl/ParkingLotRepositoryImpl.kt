...
class ParkingLotRepositoryImpl @Inject constructor(private val parkingLotDataProvider: ParkingLotDataProvider) :
    ...
    override suspend fun getOneParkingLot(): Flow<Int> = flow<Int> {
        parkingLotDataProvider.getAllParkingLot(pageNo = 1, numOfRows = 5, parkingchrgeInfo = "무료")
            .collect {
                if (it.isSuccessful) {
                    emit(it.body()!!.response?.body?.totalCount!!.toInt())
                }
            }
    }
            ...
            if (result.isSuccessful && result.body() != null) {
                result.body()!!.response.body.items.asFlow().flowOn(defaultDispatcher)
                    .filter { item ->
                        ...
                        Log.d("whatisthis", "${item} + ${inLatRange && inLonRange && inTime}")
                        inLatRange && inLonRange && inTime
                    }.collect { item ->
                        freeParkingLot.add(item.toParkingLotItem())
                    }
                Log.d("whatisthis", freeParkingLot.toString() + " okay333" + (result.body() != null))
                emit(freeParkingLot)
            }
        }
